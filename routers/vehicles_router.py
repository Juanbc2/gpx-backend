...
def get_vehicles(db: Session= Depends(get_db)):
    vehicles = vehicle_model.get_vehicles(db=db)
...
def get_vehicle(vehicle_id: str,db: Session= Depends(get_db)):
    vehicle = vehicle_model.get_vehicle_by_id(db=db, vehicle_id=vehicle_id)
...
def add_vehicle(vehicle: vehicle_schema.Vehicle,db: Session= Depends(get_db)):
    new_vehicle = vehicle_model.create_vehicle(db=db, vehicle=vehicle)
...
def get_vehicle_by_competitor(competitor_id: str,db: Session= Depends(get_db)):
    vehicle = vehicle_model.get_vehicle_by_competitor(db=db, competitor_id=competitor_id)
    if vehicle is not None:
        return vehicle
    else:
        raise HTTPException(status_code=404, detail="Vehicle not found")
...
def update_vehicle(vehicle_id: str, vehicle: vehicle_schema.Vehicle,db: Session= Depends(get_db)):
    updated_vehicle = vehicle_model.update_vehicle_by_id(db=db, vehicle_id=vehicle_id, vehicle=vehicle)
    if updated_vehicle:
...
def delete_vehicle(vehicle_id: str,db: Session= Depends(get_db)):
    vehicle = vehicle_model.delete_vehicle(db=db, vehicle_id=vehicle_id)
    if vehicle is not None:
